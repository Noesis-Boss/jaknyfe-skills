 ...
 *
 * - Uses @xenova/transformers v3 (preferred; matches upstream README)
...
const CACHE_DIR = process.env.HF_HOME || "/home/.z/hf-cache";
...
try {
  // @ts-ignore
  const v3 = await import("@xenova/transformers");
  pipeline = v3.pipeline;
  layer_norm = v3.layer_norm;
  if (!layer_norm) console.warn("⚠️  @xenova/transformers has no layer_norm export; using fallback L2-norm only");
} catch (e) {
  console.warn("⚠️  @xenova/transformers (forced), falling back to @xenova/transformers:", (e as Error).message);
  // @ts-ignore
...
async function main() {
  ...
  switch (cmd) {
    case "info": {
      ...
      console.log(`Runtime:  ${layer_norm ? "@xenova/transformers v3" : "@xenova/transformers v2"}`);
      const t0 = Date.now();
    ...
    case "embed-docs-json": {
      const texts = JSON.parse(await Bun.stdin.text()) as string[];
      const vectors: number[][] = [];
      for (const text of texts) {
        vectors.push(Array.from(await embedDoc(text)));
      }
    ...
    default:
      ...
      console.log("  embed.ts embed-docs-json  # JSON string array on stdin");
  }
}
