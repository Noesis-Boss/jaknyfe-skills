...
description: Orchestrator for the 5-stage faceless YouTube system. Run niche-brand → content-engine → discoverability → retention-script → monetization in order, writing numbered files into a per-channel project directory.
...
Orchestrate the 5 stages of the faceless-youtube-system. Each stage is its own skill; this one owns the order, the output directory, and the handoff contract between stages.
...
# 2. Run the 5 stages in order. Each stage reads the previous stage's output.
...
The runner checks that the required previous-stage output exists before invoking a stage, so you cannot run stage 4 before stage 1.
...
Almost never. The handoff contract is the point — each stage's output is the next stage's input. Reordering produces calendars that ignore the brand, or scripts that ignore the packaging.

The one exception: **stage 4 (script) can run in parallel for multiple videos** once stages 1, 2, and 3 are complete. Each script is independent.
...
- `Skills/faceless-explainer/` — render layer; takes a stage-4 script and produces video frames
