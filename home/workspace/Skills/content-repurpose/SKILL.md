...
- **content-creator** -- Content analysis, derivative content writing, format adaptation, voice alignment, editorial calendar planning, and cross-linking strategy- **content-creator** -- Content analysis, derivative content writing, format adaptation, voice alignment, editorial calendar planning, and cross-linking strategy

## LinkedIn single-video repurposing (six-angle method)

When a target channel is LinkedIn (or any text-first professional feed), do not produce one summary post. Produce **six angles from the same source video**, publish across separate slots, and let engagement data pick the winner:

1. **Story** — the personal narrative behind the video's core lesson.
2. **Contrarian take** — the belief the video argues against, stated as a position.
3. **List** — the N steps/lessons, one line each.
4. **Case study / proof** — a specific result with numbers, before → after.
5. **Question** — the problem the video solves, posed to the reader's situation.
6. **Mirror** — "you know that moment when..." — recognition of the reader's exact situation.

Rules for each angle:

- **No emojis, no dash lists.** Both are AI tells that mark the post as machine-written; they measurably depress engagement on professional feeds. Plain sentences and line breaks only.
- **First line is the hook and must stand alone** — LinkedIn truncates after ~2–3 lines.
- **Vary structure between angles**, not just wording. Six posts with the same skeleton is one post printed six times.

## Inbound/outbound loop (zero-follower monetization)

For accounts with no audience yet, repurposing is also prospecting:

- **Publish the six angles**; this is the inbound half — credibility content that makes the creator findable.
- **Track who engages** (reacts, comments, shares) on each angle; these are warm leads, not vanity metrics.
- **Outbound half** — within 24–48h, reach out personally to engaged people whose profile matches the offer. Reference the specific post they engaged with; never open with a pitch template.
- Angle-level engagement data then decides which angle becomes the default repurposing format for the next source video.
