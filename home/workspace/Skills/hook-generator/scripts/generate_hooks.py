...
Variety rules:
  ...
  - Each hook includes trigger, rationale, format, and retention bridge
  - Optional 20-hook strategy mode with ranked top five
"""
...
TRIGGER_BY_STYLE = {
    "curiosity": "curiosity gap", "contrarian": "contrarian opinion",
    "stat": "specificity", "story": "open loop", "bold claim": "unexpected insight",
    "question": "open loop", "confession": "mistake", "specific": "specificity",
    "list": "pattern recognition", "observation": "authority",
    "comparison": "unexpected comparison", "warning": "pain point",
    "forecast": "transformation",
}

FORMAT_BY_STYLE = {
    "story": "video / reel", "specific": "video / reel", "question": "carousel / text",
    "list": "carousel / text", "stat": "carousel / text", "forecast": "video / text",
}

def _hook_metadata(style, text, audience, goal, limit):
    audience_text = audience or "the target audience"
    trigger = TRIGGER_BY_STYLE.get(style, "relevance")
    fmt = FORMAT_BY_STYLE.get(style, "text / video")
    reasons = {
        "curiosity gap": f"It withholds the key explanation, making {audience_text} want the missing context.",
        "contrarian opinion": f"It challenges a familiar belief and gives {audience_text} a reason to test the claim.",
        "specificity": f"Concrete detail makes the idea easier for {audience_text} to believe and remember.",
        "open loop": f"The unresolved question gives {audience_text} a clear reason to keep consuming.",
        "mistake": f"A candid cost or failure builds credibility with {audience_text}.",
        "pain point": f"It names a risk {audience_text} already feels and promises a useful diagnosis.",
        "transformation": f"It frames a change in outcomes, helping {audience_text} see why the topic matters now.",
    }
    why = reasons.get(trigger, f"It creates a relevant tension for {audience_text}.")
    bridge = "Reveal the evidence, example, or mechanism behind the opening before giving the conclusion."
    if goal == "act": bridge = "Move from the tension to one concrete action the audience can take today."
    elif goal == "click": bridge = "Delay the full explanation until the audience sees the strongest supporting example."
    elif goal == "share": bridge = "Give the audience a quotable takeaway they can repeat after the proof lands."
    return {"psychological_trigger": trigger, "why_it_works": why,
            "best_format": fmt, "retention_bridge": bridge,
            "within_surface_limit": len(text) <= limit}
...
def _pick_styles(rng, count):
    ...
    while len(styles) < count:
        batch = list(STYLE_ORDER)
        rng.shuffle(batch)
        if styles and batch[0] == styles[-1] and len(batch) > 1:
...
def generate(topic, surface="x", audience="", goal="", count=5, seed=None):
    ...
    for i, style in enumerate(styles):
        ...
        item.update(_hook_metadata(style, h, audience, goal, limit))
        hooks.append(item)
    ...
    result = {
        ...
        "recommendation": {
            "pick": rec["style"],
            "text": rec["text"],
            "why": ("Best match for goal='%s', audience='%s', and surface='%s'" % (goal or "any", audience or "general", surface)),
        },
        "second_line": second,
    }
    ranked = sorted(hooks, key=lambda h: (
        not h["within_surface_limit"],
        -(1 if h["style"] in preferred else 0),
        h["char_count"],
    ))[:5]
    result["top_five"] = [{"rank": i, "style": h["style"], "text": h["text"],
...
def render_text(out):
    ...
    for i, h in enumerate(out["hooks"], 1):
        ...
        lines += [f'HOOK {i} - [{h["style"]}]{over}',
                  f'"{h["text"]}"',
                  f"Length: {h['char_count']} | Trigger: {h['psychological_trigger']} | Format: {h['best_format']}",
                  f"Why: {h['why_it_works']}",
                  f"Retention: {h['retention_bridge']}", ""]
    rec = out["recommendation"]
    lines += [f'RECOMMENDED: {rec["pick"]} - "{rec["text"]}"',
              f'Why: {rec["why"]}', "",
              f'SECOND LINE: "{out["second_line"]}"', "",
              "TOP 5 TO PUBLISH FIRST"]
    for item in out["top_five"]:
        lines.append(f'{item["rank"]}. [{item["style"]}] "{item["text"]}" — {item["why_it_works"]}')
    return "\n".join(lines)
