...
import { injectTracking, unsubscribeUrl } from "../tracking/tokens";
...
export async function processQueuedSend(database: Database, job: QueuedSendJob): Promise<SendAttempt> {
  ...
  try {
    const result = await sendWithAccount(database, job.organizationId, job.sendingAccountId, { to: database.query<{ email: string }, [string, string]>("SELECT email FROM contacts WHERE id = ? AND organization_id = ?").get(job.contactId, job.organizationId)!.email, subject: job.subject, body: injectTracking(job.body, messageId), headers: { "List-Unsubscribe": `<${unsubscribeUrl(messageId)}>`, "List-Unsubscribe-Post": "List-Unsubscribe=One-Click" }, from: "" });
...
export async function processQueuedSendPostgres(database: PostgresDatabase, job: QueuedSendJob): Promise<SendAttempt> {
  ...
  try {
    const result = await sendWithAccountPostgres(database, job.organizationId, job.sendingAccountId, { to: contact.email, subject: job.subject, body: injectTracking(job.body, messageId), headers: { "List-Unsubscribe": `<${unsubscribeUrl(messageId)}>`, "List-Unsubscribe-Post": "List-Unsubscribe=One-Click" }, from: "" });
