...
## Feature Log
- 2026-09-03: Added RFC 8058 one-click unsubscribe: List-Unsubscribe and List-Unsubscribe-Post headers on all outbound sends (Gmail MIME + Microsoft Graph), POST handler on /api/t/u/:token for mailbox-native unsubscribe, and tenant brand name on the unsubscribe confirmation page. Fixed a latent Postgres 42P18 crash in recordPublicEvent (unused $2 param). bun test 37/37; republished; live GET page screenshot-verified showing brand, POST one-click verified against Neon (event written, suppression enforced).
- 2026-09-03: Shipped one-click unsubscribe: HMAC purpose-separated unsubscribe tokens, styled "Unsubscribe" footer injected into every outbound HTML send, public `/api/t/u/:token` route writing suppression-list entry (reason `one_click_unsubscribe`) plus an `unsubscribed` event, and worker eligibility already enforcing suppression on future sends. Fixed the live-send-test event insert (was failing on `contact_id` FK). `bun test` 36/36; server and client bundles build; republished and screenshot-verified the live sign-in gate, and curl-verified the unsubscribe page (200 HTML). Commit `9186709`.
- 2026-09-03: Added opt-in live end-to-end send test `scripts/live-send-test.ts` (requires `LIVE_SEND_TEST=1`). Verified against production: Gmail account `noesismailer@gmail.com` sent to `don@noesisgroup.com`, provider message ID `1a0689d9efbd7ef4`. Commit `bb753dc`.

- 2026-09-03: Shipped tokenized unauthenticated tracking: HMAC-signed open/click tokens (`src/server/tracking/tokens.ts`), public `/api/t/o/:token` and `/api/t/c/:token` routes, click redirect page, tracking-pixel injection in outbound HTML, and per-campaign open/click totals with analytics bars in the dashboard. `bun test` 34/34; server and client bundles build; live site republished and screenshot-verified at the sign-in gate. Commit `66eaf52`.- 2026-09-03: Added dashboard scheduling for approved newsletters, including queue confirmation and scheduled-time display, plus per-campaign analytics summaries. `bun test` 32/32 and server bundle pass; Vite build remains blocked by the existing missing `index.html`; local browser screenshots verify the dashboard and newsletter composer render.
