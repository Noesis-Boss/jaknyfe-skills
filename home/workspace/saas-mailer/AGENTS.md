...
- 2026-09-03: Fixed scheduled sends on SQLite: the `messages` table lacked `next_attempt_at`, `lease_owner`, and `lease_until`, so the schedule route's queue INSERT failed with a 400. Added `db/migrations/010_worker_queue_columns.sql` (mirrors the Postgres queue schema) and an end-to-end test `tests/worker/scheduled-send.test.ts` covering the unapproved-schedule gate, approve → enroll → schedule (1 queued), idempotent re-schedule (0 queued), and queued-row fields. `bun test` 38/38; server bundle builds; site republished. Commit `ce368ff`.

- 2026-09-03: Shipped RFC 8058 one-click unsubscribe compliance: every outbound message now carries `List-Unsubscribe` (mailto + tokenized HTTPS) and `List-Unsubscribe-Post` headers through Gmail (MIME) and Microsoft Graph (`internetMessageHeaders`) adapters; `POST /api/t/u/:token` handles Gmail/Outlook one-click POSTs with idempotent suppression. The unsubscribe confirmation page now shows the tenant brand name ("You're unsubscribed from <org>"), and a pre-existing Postgres 42P18 error in `recordPublicEvent` (unused `$2` param) was fixed. `bun test` 37/37; live POST verified (200, idempotent, 2 unsubscribed events, suppression row present) and brand page screenshot-verified. Commit `595fe4a`.
...
## Feature Log
- 2026-09-04: Added authenticated `/api/worker/health` queue-state reporting and a dashboard Worker health pill (`ready`, `sending`, or `idle`). Production service restarted and worker service reported RUNNING. Live scheduled-send verification queued an approved message, the worker claimed it, Gmail accepted it as provider message `1a06a74d040156bb`, and the row reached `sent` in 2.5 seconds.
- 2026-09-03: Shipped tokenized unauthenticated tracking: HMAC-signed open/click tokens (`src/server/tracking/tokens.ts`), public `/api/t/o/:token` and `/api/t/c/:token` routes, click redirect page, tracking-pixel injection in outbound HTML, and per-campaign open/click totals with analytics bars in the dashboard. `bun test` 34/34; server and client bundles build; live site republished and screenshot-verified at the sign-in gate. Commit `66eaf52`.
