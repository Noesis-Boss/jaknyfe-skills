...
- 2026-09-03: Added server-issued wallet session tokens. `/api/wallet/verify` now returns an opaque token, `/api/wallet/session` validates it, and campaign creation requires its bearer authorization; Dashboard restores sessions without treating local storage as proof. Build and fresh Dashboard screenshot passed at `coinbackers-session-token-verified.png`.
...
- 2026-09-03: Implemented wallet-signed pledges and social sharing. Server: added `/api/campaigns/:id/pledge/challenge` (5-minute nonce; message binds campaign title, id, and USD amount) and hardened `POST /api/campaigns/:id/pledge` to require `personal_sign`-verified ownership via ethers `verifyMessage`, consuming one-time challenges and recording per-pledge `{amount, backerWallet, at}` entries. Client: pledge confirmation now requests `eth_requestAccounts` + `personal_sign`; environments without a provider see a clear "wallet required" error. Added X and Telegram share buttons to campaign detail (brand SVGs; Telegram uses inline hex background because Tailwind was not generating `bg-sky-500` in this build). E2E: signed pledge returned 200 and was recorded ($12,501.23, 143 backers, backerWallet logged); tampered signature rejected with 401. Share row screenshot-verified live at `cb-share-v6.png`. Committed and pushed as `4ba5ba0`.
