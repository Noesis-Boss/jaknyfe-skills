...
## 3. Invariant Labs / Snyk (MCP-Scan authors)
- **Who/what:** Invariant Labs — makers of MCP-Scan, open-source MCP security scanner. Verified: [invariantlabs.ai/blog/introducing-mcp-scan](https://invariantlabs.ai/blog/introducing-mcp-scan) (2025-04-11). **S4 correction (2026-09-07):** the repo [github.com/invariantlabs-ai/mcp-scan](https://github.com/invariantlabs-ai/mcp-scan) now redirects to `snyk/agent-scan` — Invariant has been absorbed into Snyk and the tool is branded "Agent Scan"; the README's integration section (accessed 2026-09-07) directs registry integrations to [evo.ai.snyk.io/#contact-us](https://evo.ai.snyk.io/#contact-us), which publishes "designated APIs" for including scan results in a project/registry. The previously claimed `mcpscan@invariantlabs.ai` address is NOT present in the live README and is withdrawn.
- **Why them:** precedent match — they already partner with a registry (Smithery integrates MCP-Scan results into server pages; [invariantlabs.ai/blog/smithery-mcp-scan](https://invariantlabs.ai/blog/smithery-mcp-scan), accessed 2026-09-07). The README explicitly invites registry-like projects integrating scan results to reach out via the Snyk Evo contact form.
- **Channel:** [evo.ai.snyk.io/#contact-us](https://evo.ai.snyk.io/#contact-us) (README-designated integration contact) or GitHub issues on the snyk/agent-scan repo.
- **Draft opener:** "Building a local MCP catalog gateway that attaches trust metadata to registry-mirrored entries. We'd consume Agent Scan (formerly MCP-Scan) results as one input (sourced, dated, never auto-blocking — allowlist decisions stay human). Your Smithery integration is the pattern. Is there a supported way to consume scan output programmatically for a local, non-hosted registry?"
...
3. Invariant Labs / Snyk (README-designated integration contact form, exact-precedent ask)
