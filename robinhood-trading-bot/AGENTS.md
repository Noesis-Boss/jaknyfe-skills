...
- 2026-09-12: Added two archify diagrams to `docs/` and linked them from this file. `docs/backtest-dataflow.html` (commit 9e5e6fc) shows the end-to-end pipeline: Alpaca 5m bars → premarket box → breakout gate → fill → position → expiry/stop → journal → next bar, with theta-farming side branch and realism knobs. `docs/strategy-flow.html` (commit 9d377bcd) shows the per-bar London breakout flow: Market Data → Premarket Box → Entry Window/Volume/Bias gates → Enter Trade → Manage+Journal, with the Theta Farmer side branch. Both built from real code in `src/strategy.py` + `src/theta_farming.py` + `src/backtest_runner.py`; light + dark visual checks pass at 1440x900 and 2048x1320.
