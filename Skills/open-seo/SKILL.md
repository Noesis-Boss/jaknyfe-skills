...
## Free fallbacks (no OpenSEO/DataForSEO credits)

The self-hosted instance (`Projects/open-seo`) degrades gracefully when `DATAFORSEO_API_KEY` is absent — tools state `DATAFORSEO_AUTH_FAILED` and fall back to free providers instead of failing:

- **Keyword research / ranked keywords / backlinks / rank checks** → Bing Webmaster Tools (`BING_WEBMASTER_API_KEY`). Requires the domain to be registered in Bing Webmaster Tools; Bing rows are impressions/clicks and impression-weighted average position, not Google search volume or exact ranks.
- **Lighthouse site-audit checks** → Google PageSpeed Insights (`PAGESPEED_API_KEY` optional). PSI runs real Lighthouse; payloads are labeled `pagespeed-insights` in stored data.
- Both fallbacks return null/empty when their key is missing or the domain is unregistered — the original DataForSEO error is kept so the user can act on it.
