...
### X/Twitter (`use_app_x`)
> **First-line rule:** Before drafting the launch thread, run the hook-generator skill to produce the opening tweet. The first tweet of any launch thread is a top-of-copy surface — it decides whether anyone reads the rest. Generate with:
> ```bash
> python3 /home/workspace/Skills/hook-generator/scripts/generate_hooks.py "TOPIC" --surface x --audience "AUDIENCE" --goal click
> ```
> Use the recommended hook as tweet 1. Subsequent tweets are body — no hook needed. Skip this only if Don has explicitly written the first line himself.
