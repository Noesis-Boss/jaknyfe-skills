...
The skill ships a small TypeScript CLI (`file scripts/main.ts`). Run it with `bun run scripts/main.ts <action>` where `<action>` is one of `characters`, `outline`, or `draft`.
