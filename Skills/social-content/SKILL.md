...
metadata:
  ...
  category: External
  display-name: Social Content Expert
---
...
> **First-line rule:** When creating an original tweet or the first tweet of a thread, invoke the hook-generator skill to draft the opener — the first line decides whether anyone reads the rest. Generate with:
> ```bash
> python3 /home/workspace/Skills/hook-generator/scripts/generate_hooks.py "TOPIC" --surface x --audience "AUDIENCE" --goal GOAL
> ```
> Use the recommended hook (or a variant that fits the body) as tweet 1. Skip this for replies and quote-tweets (the original post IS the hook).
