...
13. Regression fixture check (borrowed from SlopMonster): keep a small set of known human passages and known failure passages for each new detector rule. A rule is useful only when it catches the failure passage without flagging the human passage. Re-run the fixtures after changing thresholds or pattern lists.
...
v3 additions (2026-09-04, blader/humanizer): "gate/gated/gating" (figurative — preserve established technical usage), "highlight" (verb), "quietly," "key" (adjective), "actually."
...
**17b. Repeated sentence openings**

AI handles repetition by rule instead of by ear: it keeps renaming the same subject (synonym cycling — "the protagonist... the main character... the central figure... the hero") or starts several consecutive sentences with the same subject ("She noted the door. She noted the lock on it. She filed both away.").

Fix: use one clear name for the same subject. Merge repeated-opening sentences, change the subject when it helps, or begin with the action. Do not ban the repeated word — fix the repeated sentence pattern; the remaining sentence may still start with "She."
...
- Specific, falsifiable details: names, dates, counts, prices, quotes — the things a model would have to invent- Mixed feelings and unresolved tension: "I think this is mostly good, but it bothers me, and I can't fully explain why."
- Dated, era-bound references: slang, memes, or in-jokes tied to a specific year and subculture (models lag a year or more)
- Deliberate first-person choices the writer can explain
- Variety in sentence length — real writing alternates short and long; AI prose drifts toward an even, mid-length cadence
- Genuine asides, parentheticals, and self-corrections: "(I keep wanting to say 'almost' here, but it really was certain.)"
- Edits made before November 30, 2022 (ChatGPT's public launch) — with rare exceptions, not AI-written
