...
async function completeTask(taskId: string, companyId: string, result: string): Promise<void> {
  const response = await fetch(`${API_BASE}/api/tasks/${taskId}/complete`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders() },
    body: JSON.stringify({ result }),
  });
  if (!response.ok) console.error(`[${taskId}] Complete failed: ${await response.text()}`);
}
