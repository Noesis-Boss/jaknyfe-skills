...
We use 8–10 templates per style, a random seed per call, and rotate number/time/place pools. An anti-repetition guard ensures no duplicate phrasing across variants.
...
- **Curiosity** — open a loop the reader wants closed. The "I was wrong about X" confession as a default opener has overstayed its welcome across X. Rotate between concrete payoffs, missing-context reveals, and surprise numbers instead.
  - **Curiosity vs. tension:** curiosity is "what happened next?" — a question the reader wants answered. Tension is "wait, what?" — a momentary jolt of cognitive friction. On short-form, fast-scroll surfaces (X, YouTube Shorts, TikTok), tension stops the scroll; curiosity holds attention. Lead with tension when the audience is in scroll mode, curiosity when they've already committed (article, newsletter, podcast cold open).
...
- **Bold claim** — make a claim so strong it's either clearly right or clearly wrong. "Reading 50 papers a week made me worse at my job for a year." Commitment forces engagement. The strongest version of this style says something that *feels illegal* — a stance the reader is mildly surprised is allowed in public. Force is what makes it land; politeness dilutes it.
...
- **Outcome-first** — lead with the *result*, not the setup. "This made $47 while I slept" beats "I started a faceless channel and…" Backstory is what you cut; outcome is what earns the click. Use when the body delivers the receipt for the claim.
- **But / Therefore frame** — a three-beat opener: "People think X. **But** that's wrong. **Therefore**, do Y." Works as a hook on its own, or as the structure of the *second line* that connects a curiosity/tension hook to the body. Don't use the full three beats as a hook on fast surfaces — it reads slow. Use one beat (the "But") instead.

## Tension & interruption (the scroll-stopping principle)

Hooks on short-form, fast-scroll surfaces are doing a different job than hooks on slow surfaces. The job is not to *invite* — it's to *interrupt*. Two related ideas that show up in viral short-form:

- **Tension > curiosity** when the audience hasn't committed yet. A jolt of "wait, what?" beats "what happens next?" for stopping a thumb.
- **Interrupt, don't invite.** Pretty is forgettable. Ugly-bold text, sudden zoom, hard cuts to movement, visual contradictions (money + boredom, luxury + chaos) are patterns that interrupt the scroll pattern. Translate to text surfaces as: slightly uncomfortable framing, the first three words doing something the reader didn't expect, a stance that feels mildly forbidden.

For short-form video specifically, the hook is the *first frame*, not the script. The visual earns the second look; the script earns the watch-through. If you're generating a hook for the caption/voiceover of a Short, also recommend a first-frame visual the editor can pair it with.
...
- When the post is going to a fast-moving feed, lead with tension or outcome; reserve pure curiosity for threads where the reader has already committed to the first tweet.
...
- **Default to outcome-first** for posts that exist to teach a tactic or show a result. The first paragraph is the receipt, not the introduction. Save "I started doing X" for case studies where the journey *is* the content.
...
### YouTube Shorts / TikTok (first 1.5 seconds, ≤100 chars voiceover)
- The first frame does the scroll-stopping, not the script. Pair any text hook with a visual recommendation: sudden zoom (110–120%), hard cut to motion, slightly-too-bold text, or a visual contradiction (luxury setting + chaotic action).
- Lead with outcome-first or "feels illegal" stance. Backstory loses the thumb.
- Voiceover ≤ 8 words. If the hook needs more than that, it's not a hook — it's a sentence.

### Newsletter subject
- Lowercase, conversational, 4–7 words is the proven sweet spot. "the quietest signal in the market this week"

## Pre-publish self-check (use before recommending)

Before recommending a hook, run these three checks. If any answer is "no," either revise the hook or pick a different one:

1. **Would this stop *me* from scrolling?** If not, it's not sharp enough.
2. **Is the first frame (for video) or the first three words (for text) doing something the reader didn't expect?** If it's predictable, it's invisible.
3. **Does the opening feel slightly risky to publish?** Polite rarely gets replayed. Safe is invisible. If it feels too safe, the reader will scroll past it too.
...
2. Decide the surface (X, article, subject, Shorts, etc.) — if unclear, default to X and label it.
3. Decide the mode: scroll-stop (tension + interruption, short-form) or committed-read (curiosity loop, long-form). Default to scroll-stop for X, Shorts, TikTok, cold email subject; default to committed-read for article intro, newsletter, podcast cold open.
4. Generate 5 hooks across 5 different styles from the toolkit.
5. Run the pre-publish self-check on each. Eliminate any that fail it, or note which check failed.
6. Pick a recommendation with a one-line reason.
7. If the surface is short-form video, include a paired first-frame visual suggestion under SECOND LINE.
8. If the user is on a tight character budget, also include a character count for each hook.
9. Keep the output short — a hook is not an essay. Total response should fit on one screen.
