...
export function createStdoutParser() {
    ...
    function isToolInvocationLine(line) {
        ...
        if (/^(?:cat|less|more|head|tail|grep|egrep|fgrep|sed|awk|find|ls|cd|mkdir|rmdir|rm|mv|cp|chmod|chown|touch|ln|stat|file|wc|sort|uniq|diff|tee|xargs|cut|tr|echo|source|export|env|which|pwd|tar|zip|unzip)\b/.test(line) && /\s/.test(line))
            return true;
    ...
    function classifyDiffLine(trimmed) {
        // Hunk header: @@ -X,Y +X,Y @@
...
export const defaultParser = createStdoutParser();
/**
