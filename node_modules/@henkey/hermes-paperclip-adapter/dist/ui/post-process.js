 ...
 */
export function postProcessMessage(text) {
...
function isHighlightSeparator(trimmed) {
    if (trimmed.length < 8)
        return false;
    if (!/^[\s═=─\u2500\u2501\u2013\u2014-]+$/.test(trimmed))
        return false;
...
function convertHermesLines(lines) {
    const result = [];
 ...
 */
function normalizeBorderDashes(pipeRow) {
    const t = pipeRow.trim();
    if (!isDelimiterOnlyRow(t))
        return pipeRow;
 ...
 */
function isDelimiterOnlyRow(trimmed) {
 ...
 */
function firstInnerDelimiterIndex(buffer) {
    for (let i = 1; i < buffer.length; i++) {
        if (!isDelimiterOnlyRow(buffer[i].trim()))
            continue;
        if (isDelimiterOnlyRow(buffer[i - 1].trim()))
            continue;
        return i;
    }
    return -1;
 ...
 */
function findClosingBorderSplitIndex(buffer) {
 ...
 */
function splitContiguousPipeBuffer(buffer) {
    const parts = [];
    let rest = buffer;
    while (rest.length > 0) {
 ...
 */
function normalizeHermesTableBlock(block) {
    if (block.length === 0)
        return block;
    const lines = [...block];
...
function normalizeTableRegions(lines) {
    const out = [];
    let i = 0;
    while (i < lines.length) {
...
function findPrevNonEmpty(lines) {
