 ...
 */
import { buildPaperclipEnv, renderTemplate, ensureAbsoluteDirectory, appendWithCap, runningProcesses, } from "@paperclipai/adapter-utils/server-utils";
import { HERMES_CLI, DEFAULT_TIMEOUT_SEC, DEFAULT_GRACE_SEC, DEFAULT_IDLE_TIMEOUT_SEC, DEFAULT_MODEL, DEFAULT_DELIVERY_TARGET, DEFAULT_MEMORY_SCOPE, VALID_DELIVERY_TARGETS, DEFAULT_RESUME_STRATEGY, } from "../shared/constants.js";
...
// ---------------------------------------------------------------------------
function isToolInvocationNoise(line) {
    // Bare shell/tool commands that Hermes outputs as quiet-mode tool arguments.
    // These are invocation noise, not assistant prose.
    if (/^(?:curl|wget|ssh|scp|rsync)\b/.test(line) && / -[A-Za-z]/.test(line))
        return true;
    if (/^(?:git|npm|bun|yarn|pnpm|docker|kubectl|aws|gh)\b/.test(line) && /\s/.test(line))
        return true;
    if (/^(?:python3?|node|npx|pip3?)\s/.test(line))
        return true;
    if (/^(?:cat|less|head|tail|grep|egrep|sed|awk|find|ls|cd|mkdir|rm|mv|cp|chmod|chown|touch|ln|stat|file|wc|sort|uniq|diff|tee|xargs|cut|tr|echo|source|export|env|which|pwd|tar|zip|unzip)\b/.test(line) && /\s/.test(line))
        return true;
 ...
 */
function extractFinalResponseBlock(stdout) {
 ...
 */
function readSessionUsageFromDb(sessionId, hermesHomeOverride) {
    if (!sessionId)
        return null;
    try {
        ...
        const cacheReadTokens = data.cache_read_tokens || 0;
        ...
        return {
            ...
            costUsd: data.actual_cost_usd || data.estimated_cost_usd || 0,
        };
    }
    catch {
 ...
 */
function runChildProcessWithIdleTimeout(runId, command, args, opts) {
    return new Promise((resolve, reject) => {
        ...
        const isBenignStartupLine = (trimmed) => /^\[?\d{4}[-/]\d{2}[-/]\d{2}T/.test(trimmed) ||
            /^[A-Z]+:\s+(INFO|DEBUG|WARN|WARNING)\b/.test(trimmed) ||
            /Successfully registered all tools/.test(trimmed) ||
            /MCP [Ss]erver/.test(trimmed) ||
            /tool registered successfully/.test(trimmed) ||
            /Application initialized/.test(trimmed);
        const chunkHasMeaningfulOutput = (text) => {
            for (const line of text.split(/\r?\n/)) {
                const trimmed = line.trim();
                if (!trimmed)
                    continue;
                if (isBenignStartupLine(trimmed))
                    continue;
                return true;
            }
            return false;
        };
        const scheduleIdleTimer = (timeoutSec, reason) => {
            if (idleTimer)
        ...
        const resetIdleTimer = () => {
            scheduleIdleTimer(sawMeaningfulOutput ? opts.idleTimeoutSec : initialResponseIdleTimeoutSec, sawMeaningfulOutput ? "regular" : "initial");
        };
        ...
        const updateIdleTimerForChunk = (text) => {
...
export async function execute(ctx) {
    ...
    let profileDetectedConfig = null;
    let defaultDetectedConfig = null;
    ...
    const detectedConfigs = [
        profileDetectedConfig
            ? {
                ...profileDetectedConfig,
                source: profileName && profileName !== "default"
                    ? `profile:${profileName}`
                    : "defaultConfig",
            }
            : null,
    ...
    if (instructionsFilePath) {
        ...
        const instrCwd = cfgString(ctx.config?.workspaceDir) || cfgString(config.cwd) || ".";
        ...
        try {
            ...
            await ctx.onLog("stdout", `[paperclip] Warning: could not read agent instructions file "${resolvedPath}": ${reason}\n`);
        }
    }
    ...
    if (profileName && profileName !== "default") {
        ...
        if (profilePath) {
            ...
            await ctx.onLog("stdout", `[hermes] Using profile: ${profileName} (${profilePath})\n`);
        }
        else {
            await ctx.onLog("stdout", `[hermes] Warning: profile "${profileName}" could not be created, falling back to default\n`);
        }
    }
    ...
    if (cfgBoolean(config.worktreeMode) === true)
        args.push("-w");
    if (cfgBoolean(config.checkpoints) === true)
    ...
    const prevSessionParams = ctx.runtime?.sessionParams;
    ...
    if (prevSessionId && !shouldResume) {
        await ctx.onLog("stdout", `[hermes] Smart resume: NOT resuming session ${prevSessionId}. Reason: ${resumeReason}\n`);
    }
    if (extraArgs?.length) {
        args.push(...extraArgs);
    }
    ...
    if (deliveryTarget && deliveryTarget !== "none" && VALID_DELIVERY_TARGETS.includes(deliveryTarget)) {
    ...
    const cwd = cfgString(ctx.config?.workspaceDir) || cfgString(config.cwd) || ".";
    try {
    ...
    const commandNotes = [];
    if (model)
        commandNotes.push(`Model: ${model} (provider: ${resolvedProvider} [${resolvedFrom}])`);
    if (profileName && profileName !== "default")
        commandNotes.push(`Profile: ${profileName}`);
    if (toolsets)
        commandNotes.push(`Toolsets: ${toolsets}`);
    commandNotes.push(`Memory: ${memoryScope}${deliveryTarget !== "none" ? ` → ${deliveryTarget}` : ""}`);
    if (instructionsFilePath)
        commandNotes.push(`Instructions: ${instructionsFilePath}`);
    if (prevSessionId)
        commandNotes.push(`Resuming session: ${prevSessionId}`);
    if (ctx.onMeta) {
        await ctx.onMeta({
    ...
    if (dbUsage?.usage) {
        await ctx.onLog("stdout", `[hermes] Usage (from session DB): ${dbUsage.usage.inputTokens} input, ${dbUsage.usage.outputTokens} output${dbUsage.usage.cachedInputTokens ? `, ${dbUsage.usage.cachedInputTokens} cached` : ""}\n`);
    }
    // ── Build result ───────────────────────────────────────────────────────
    ...
    if (persistSession && parsed.sessionId) {
        ...
        let previousRunOutcome;
        const CONTEXT_ERROR_PATTERNS = [
            "context", "max length", "token limit", "too long",
            "prompt exceeds", "BadRequestError",
        ];
        const hadContextError = CONTEXT_ERROR_PATTERNS.some((p) => (parsed.errorMessage || "").toLowerCase().includes(p) ||
        ...
        await ctx.onLog("stdout", `[hermes] Run outcome: ${previousRunOutcome} (exit=${result.exitCode}, signal=${result.signal}, timedOut=${result.timedOut}, idleKilled=${result.idleKilled}) → stored for next smart resume decision\n`);
    }
