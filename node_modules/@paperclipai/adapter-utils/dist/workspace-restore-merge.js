...
function shouldExclude(relative, exclude) {
...
async function walkDirectory(root, exclude, relative = "", out = new Map()) {
    const current = relative ? path.join(root, relative) : root;
    const entries = await fs.readdir(current, { withFileTypes: true }).catch(() => []);
    entries.sort((left, right) => left.name.localeCompare(right.name));
    for (const entry of entries) {
        const nextRelative = relative ? path.posix.join(relative, entry.name) : entry.name;
        if (shouldExclude(nextRelative, exclude))
            continue;
...
async function readSnapshotEntry(root, relative) {
...
export async function withDirectoryMergeLock(targetDir, fn) {
...
export async function captureDirectorySnapshot(rootDir, options = {}) {
...
export async function mergeDirectoryWithBaseline(input) {
    ...
    await withDirectoryMergeLock(input.targetDir, async () => {
        ...
        for (const [relative, baselineEntry] of deletedLeafEntries) {
            if (!entriesMatch(current.entries.get(relative), baselineEntry))
                continue;
...
export async function directoryEntryMatchesBaseline(rootDir, relative, baselineEntry) {
    return entriesMatch(await readSnapshotEntry(rootDir, relative), baselineEntry);
}
//# sourceMappingURL=workspace-restore-merge.js.map