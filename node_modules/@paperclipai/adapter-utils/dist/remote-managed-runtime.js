...
import { prepareWorkspaceForSshExecution, restoreWorkspaceFromSshExecution, syncDirectoryToSsh, } from "./ssh.js";
...
export function buildRemoteExecutionSessionIdentity(spec) {
    if (!spec)
        return null;
    return {
        transport: "ssh",
...
export function remoteExecutionSessionMatches(saved, current) {
...
export async function prepareRemoteManagedRuntime(input) {
    ...
    const workspaceRemoteDir = path.posix.join(baseWorkspaceRemoteDir, ".paperclip-runtime", "runs", input.runId, "workspace");
...
//# sourceMappingURL=remote-managed-runtime.js.map