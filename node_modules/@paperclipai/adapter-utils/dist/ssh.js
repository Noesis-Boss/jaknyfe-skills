...
async function integrateImportedGitHead(input) {
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        if (!currentHead || currentHead === input.importedHead)
            return;
        ...
        if (mergeBaseHead === currentHead) {
            try {
                ...
                if (isConcurrentRefUpdateError(error) && attempt < 4)
                    continue;
                throw error;
            }
        }
        ...
        try {
            ...
            throw new Error(`Failed to merge concurrent SSH git histories for ${currentHead.slice(0, 12)} and ${input.importedHead.slice(0, 12)}: ${reason}`);
        }
        ...
        const mergeCommit = await runLocalGit(input.localDir, [
            "commit-tree",
            mergedTreeId,
            "-p",
            currentHead,
...
export async function runSshCommand(config, remoteCommand, options = {}) {
    ...
    try {
        ...
        sshArgs.push("-p", String(config.port), `${config.username}@${config.host}`, `sh -c ${shellQuote(remoteScript)}`);
...
export async function buildSshSpawnTarget(input) {
    ...
    sshArgs.push("-p", String(input.spec.port), `${input.spec.username}@${input.spec.host}`, `sh -c ${shellQuote(remoteScript)}`);
    return {
        command: "ssh",
        args: sshArgs,
        cleanup: auth.cleanup,
...
export async function ensureSshWorkspaceReady(config) {
    const result = await runSshCommand(config, `mkdir -p ${shellQuote(config.remoteWorkspacePath)} && cd ${shellQuote(config.remoteWorkspacePath)} && pwd`);
    return {
