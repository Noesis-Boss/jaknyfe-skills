...
export function parseSandboxRemoteExecutionSpec(value) {
...
export function buildSandboxExecutionSessionIdentity(spec) {
    if (!spec)
        return null;
    return {
        transport: "sandbox",
...
export function sandboxExecutionSessionMatches(saved, current) {
    const currentIdentity = buildSandboxExecutionSessionIdentity(current);
    if (!currentIdentity)
        return false;
    const parsedSaved = asObject(saved);
    return (asString(parsedSaved.transport) === currentIdentity.transport &&
...
export async function mirrorDirectory(sourceDir, targetDir, options = {}) {
    await fs.mkdir(targetDir, { recursive: true });
    const preserveAbsent = new Set(options.preserveAbsent ?? []);
    const shouldPreserveAbsent = (relative) => [...preserveAbsent].some((candidate) => isRelativePathOrDescendant(relative, candidate));
...
function tarExcludeFlags(exclude) {
...
export async function prepareSandboxManagedRuntime(input) {
    ...
    await withTempDir("paperclip-sandbox-sync-", async (tempDir) => {
        ...
        await input.client.run(`sh -c ${shellQuote(`mkdir -p ${shellQuote(workspaceRemoteDir)} && ` +
            ...
            `rm -f ${shellQuote(remoteWorkspaceTar)}`)}`, { timeoutMs: input.spec.timeoutMs });
        for (const asset of input.assets ?? []) {
            ...
            await input.client.run(`sh -c ${shellQuote(`rm -rf ${shellQuote(remoteAssetDir)} && ` +
                `mkdir -p ${shellQuote(remoteAssetDir)} && ` +
                `tar -xf ${shellQuote(remoteAssetTar)} -C ${shellQuote(remoteAssetDir)} && ` +
                `rm -f ${shellQuote(remoteAssetTar)}`)}`, { timeoutMs: input.spec.timeoutMs });
        }
    });
    const assetDirs = Object.fromEntries((input.assets ?? []).map((asset) => [asset.key, path.posix.join(runtimeRootDir, asset.key)]));
    return {
        spec: input.spec,
        ...
        restoreWorkspace: async () => {
            await withTempDir("paperclip-sandbox-restore-", async (tempDir) => {
                ...
                await input.client.run(`sh -c ${shellQuote(`mkdir -p ${shellQuote(runtimeRootDir)} && ` +
                    `tar -cf ${shellQuote(remoteWorkspaceTar)} -C ${shellQuote(workspaceRemoteDir)} ` +
                    `${tarExcludeFlags(input.workspaceExclude)} .`)}`, { timeoutMs: input.spec.timeoutMs });
...
//# sourceMappingURL=sandbox-managed-runtime.js.map