...
export function resolveAdapterExecutionTargetTimeoutSec(target, configuredTimeoutSec) {
    const normalizedConfiguredTimeoutSec = typeof configuredTimeoutSec === "number" && Number.isFinite(configuredTimeoutSec) && configuredTimeoutSec > 0
        ? Math.floor(configuredTimeoutSec)
        : 0;
    if (normalizedConfiguredTimeoutSec > 0)
        return normalizedConfiguredTimeoutSec;
...
export async function ensureAdapterExecutionTargetCommandResolvable(command, target, cwd, env, options = {}) {
    if (target?.kind === "remote" && target.transport === "sandbox") {
        await ensureSandboxCommandResolvable(command, target, options.installCommand?.trim() || null, options.timeoutSec);
        return;
    }
...
async function ensureSandboxCommandResolvable(command, target, installCommand, timeoutSec) {
