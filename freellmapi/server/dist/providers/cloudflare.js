...
export class CloudflareProvider extends BaseProvider {
    ...
    async validateKey(apiKey, quotaContext) {
        ...
        const userResult = await this.verifyAt('https://api.cloudflare.com/client/v4/user/tokens/verify', token, quotaContext);
        if (userResult !== 'auth-failed')
            return userResult;
        const accountResult = await this.verifyAt(`https://api.cloudflare.com/client/v4/accounts/${accountId}/tokens/verify`, token, quotaContext);
        if (accountResult === 'auth-failed')
            return false;
    ...
    async verifyAt(url, token, quotaContext) {
        const res = await this.fetchWithTimeout(url, { method: 'GET', headers: { 'Authorization': `Bearer ${token}` } }, 10000);
        recordQuotaObservationsFromResponse(res, {
            platform: this.platform,
            keyId: quotaContext?.keyId,
            providerAccountId: quotaContext?.providerAccountId,
