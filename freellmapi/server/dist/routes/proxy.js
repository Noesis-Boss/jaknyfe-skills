...
import { z } from 'zod';
import { routeRequest, resolveRoutingChain, resolveModelGroupCandidates, recordRateLimitHit, recordSuccess, hasEnabledVisionModel, hasEnabledToolsModel } from '../services/router.js';
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        try {
            if (stream) {
                ...
                try {
                    const gen = route.provider.streamChatCompletion(route.apiKey, outboundMessages, route.modelId, { temperature, max_tokens, top_p, tools, tool_choice, parallel_tool_calls }, quotaContextForRoute(route, 'chat/completions'));
                ...
                const result = await route.provider.chatCompletion(route.apiKey, outboundMessages, route.modelId, { temperature, max_tokens, top_p, tools, tool_choice, parallel_tool_calls }, quotaContextForRoute(route, 'chat/completions'));
