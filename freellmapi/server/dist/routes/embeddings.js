...
import { listEmbeddingModels, getDefaultFamily } from '../services/embeddings.js';
...
embeddingsRouter.get('/', (_req, res) => {
    const db = getDb();
    const keyCounts = new Map(db.prepare("SELECT platform, COUNT(*) AS n FROM api_keys WHERE enabled = 1 AND status IN ('healthy', 'unknown') GROUP BY platform").all().map(r => [r.platform, r.n]));
...
embeddingsRouter.put('/', (req, res) => {
    ...
    if (parsed.data.providers) {
        ...
        const apply = db.transaction((rows) => {
            for (const r of rows)
                update.run(r.priority, r.enabled ? 1 : 0, r.id);
        });
...
embeddingsRouter.get('/usage', (_req, res) => {
    const db = getDb();
