    ...
    for (const message of input.messages) {
        ...
        if (message.role === 'system') {
            const sysText = typeof message.content === 'string'
                ? message.content
                : message.content.map(b => (typeof b.text === 'string' ? b.text : '')).filter(Boolean).join('\n');
            if (sysText)
...
function mapStopReason(finishReason, hadToolCalls) {
    ...
    if (finishReason === 'length')
        return 'max_tokens';
    if (finishReason === 'stop' || finishReason === 'content_filter' || finishReason == null)
        return 'end_turn';
    return 'end_turn';
}
...
function cooldownFor(route, err) {
    if (isPaymentRequiredError(err))
        return PAYMENT_REQUIRED_COOLDOWN_MS;
    if (isModelAccessForbiddenError(err))
        return MODEL_FORBIDDEN_COOLDOWN_MS;
...
anthropicRouter.post('/messages', async (req, res) => {
    ...
    const imageCount = messages.reduce((n, m) => n + (Array.isArray(m.content) ? m.content.filter(b => b?.type === 'image_url').length : 0), 0);
...
async function streamCompletion(res, route, messages, options, ctx) {
    ...
    const ensureMessageStart = () => {
        ...
        res.setHeader('X-Routed-Via', `${route.platform}/${route.modelId}`);
        if (ctx.attempt > 0)
            res.setHeader('X-Fallback-Attempts', String(ctx.attempt));
    ...
    try {
        ...
        for await (const chunk of gen) {
            ...
            for (const tc of choice.delta?.tool_calls ?? []) {
                ...
                if (!toolAcc.has(idx))
                    toolAcc.set(idx, { id: undefined, name: '', args: '' });
...
anthropicRouter.get('/models', (req, res, next) => {
    if (!req.headers['anthropic-version'])
        return next(); // OpenAI client → proxyRouter
    if (!authenticate(req, res))
        return;
    const { models } = buildModelListing();
    const data = [
        { type: 'model', id: 'auto', display_name: 'Auto (router picks the best available model)', created_at: MODEL_CREATED_AT },
        ...models
            .filter(m => m.available === 1)
            .map(m => ({ type: 'model', id: m.id, display_name: m.name, created_at: MODEL_CREATED_AT })),
    ];
