...
fallbackRouter.put('/routing', (req, res) => {
...
const INTELLIGENCE_TIER = "CASE m.size_label WHEN 'Frontier' THEN 1 WHEN 'Large' THEN 2 WHEN 'Medium' THEN 3 WHEN 'Small' THEN 4 ELSE 5 END";
...
function getBudgetScore(m) {
    if (m.tpd_limit != null)
        return m.tpd_limit * 30;
    const str = m.monthly_token_budget;
    if (!str)
        return 0;
    if (str.toLowerCase().includes('unlimited') || str.includes('∞'))
        return Infinity;
...
fallbackRouter.post('/sort/:preset', (req, res) => {
    ...
    let models = [];
    if (preset === 'budget') {
        const allModels = db.prepare(`SELECT id, monthly_token_budget, tpd_limit FROM models`).all();
        ...
        models = db.prepare(`SELECT m.id FROM models m ORDER BY ${orderBy}`).all();
    }
...
fallbackRouter.get('/token-usage', (_req, res) => {
  ...
  `).all();
    ...
    const settingRow = db.prepare(`SELECT value FROM settings WHERE key = 'active_profile_id'`).get();
    const activeProfileId = settingRow ? (parseInt(settingRow.value) || null) : null;
    // Verify active profile still exists
    const activeProfile = activeProfileId
        ? db.prepare('SELECT id FROM profiles WHERE id = ?').get(activeProfileId)
        : null;
    let rawModels;
    if (activeProfile) {
