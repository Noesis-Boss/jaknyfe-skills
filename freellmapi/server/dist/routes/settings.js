...
import { getUnifiedApiKey, regenerateUnifiedKey, setSetting } from '../db/index.js';
...
settingsRouter.put('/unify', (req, res) => {
    ...
    if (parsed.data.enabled !== undefined)
        setUnifyEnabled(parsed.data.enabled);
    if (parsed.data.overrides)
        setUnifyOverrides(parsed.data.overrides);
    res.json({ enabled: isUnifyEnabled(), overrides: getUnifyOverrides() });
});
// Get the saved fusion default config (panel mode, models, judge, k, strategy).
settingsRouter.get('/fusion', (_req, res) => {
...
settingsRouter.put('/fusion', (req, res) => {
...
settingsRouter.get('/anthropic-map', (_req, res) => {
...
settingsRouter.put('/anthropic-map', (req, res) => {
    try {
...
settingsRouter.get('/proxy', (_req, res) => {
    res.json({
...
settingsRouter.put('/proxy', (req, res) => {
    const { proxyUrl, enabled, bypassPlatforms } = req.body;
    // --- proxyUrl ---
