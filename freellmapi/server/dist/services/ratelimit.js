...
const DEFAULT_PROVIDER_DAILY_REQUEST_CAPS = {
    openrouter: 1000,
};
export function getProviderDailyRequestCap(platform) {
...
function countPersistedProviderRequests(platform, keyId, windowMs, now) {
    return withDb(db => {
        const row = db.prepare(`
      SELECT COUNT(*) AS used
        FROM rate_limit_usage
       WHERE platform = ?
         ...
         AND kind = 'request'
         AND created_at_ms > ?
    `).get(platform, keyId, now - windowMs);
        return row.used;
    });
}
// Total requests today for a provider account+key, summed across every model.
export function providerDailyRequestCount(platform, keyId, now = Date.now()) {
...
export function canUseProvider(platform, keyId, now = Date.now()) {
...
export function getCooldownDurationForLimit(platform, modelId, keyId, limits, retryAfterMs) {
    const now = Date.now();
    const rpdExhausted = limits.rpd !== null && requestCount(platform, modelId, keyId, DAY, now) >= limits.rpd;
    const tpdExhausted = limits.tpd !== null && tokenCount(platform, modelId, keyId, DAY, now) >= limits.tpd;
 ...
 */
export function parseProviderLimit(message) {
    if (!message)
        return null;
 ...
 */
export function learnLimitFromError(modelDbId, err) {
    const parsed = parseProviderLimit(err?.message);
    if (!parsed)
        return null;
    const col = LIMIT_COLUMN[parsed.kind];
    const result = withDb(db => db.prepare(`UPDATE models SET ${col} = ? WHERE id = ? AND (${col} IS NULL OR ${col} > ?)`).run(parsed.limit, modelDbId, parsed.limit));
