import { z } from 'zod';
import { routePinnedModel, routeRequest, getOrderedFusionChain, resolveFusionCandidate, recordRateLimitHit, recordSuccess, } from './router.js';
import { recordRequest, recordTokens, setCooldown, getCooldownDurationForLimit, PAYMENT_REQUIRED_COOLDOWN_MS, MODEL_FORBIDDEN_COOLDOWN_MS, } from './ratelimit.js';
        ...
        try {
            ...
            if (isRetryableError(err)) {
                if (isModelNotFoundError(err) || isModelAccessForbiddenError(err))
                    skipModels.add(route.modelDbId);
                skipKeys.add(`${route.platform}:${route.modelId}:${route.keyId}`);
                setCooldown(route.platform, route.modelId, route.keyId, isPaymentRequiredError(err) ? PAYMENT_REQUIRED_COOLDOWN_MS
...
export async function runFusion(params) {
    ...
    const runSlot = (cand) => runModelCall((skipKeys) => routePinnedModel(cand.modelDbId, estimatedTokens, skipKeys), messages, options, estimatedTokens, MAX_SLOT_ATTEMPTS).then((outcome) => {
    ...
    if (survivors.length < SYNTHESIS_QUORUM || strategy === 'best_of') {
        ...
        const getJudgeRoute = config.judge
            ...
            : (skipKeys, skipModels) => routeRequest(judgeEstimate, skipKeys.size ? skipKeys : undefined, undefined, false, false, skipModels.size ? skipModels : undefined);
