...
export function listMediaModels(modality) {
    return getDb()
...
export function listAllMediaModels() {
    return getDb()
...
async function mediaFetch(url, platform, init) {
...
function parseCfKey(key) {
    if (!key)
        throw new MediaError('cloudflare key required (account_id:token)', 401);
...
async function callImageProvider(row, key, p) {
    ...
    switch (row.platform) {
        ...
        case 'cloudflare': {
            ...
            if (ct.includes('application/json')) {
                ...
                if (!b64)
                    throw new MediaError('cloudflare returned no image', 502);
                return [{ b64_json: b64 }];
            }
            const buf = Buffer.from(await r.arrayBuffer());
            return [{ b64_json: buf.toString('base64') }];
...
async function callSpeechProvider(row, key, p) {
    switch (row.platform) {
        case 'cloudflare': {
            const { accountId, token } = parseCfKey(key);
            const r = await mediaFetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/ai/run/${row.model_id}`, 'cloudflare', {
            ...
            if (!b64)
                throw new MediaError('cloudflare returned no audio', 502);
        ...
        case 'pollinations': {
            ...
            if (!b64)
                throw new MediaError('pollinations returned no audio', 502);
        ...
        case 'google': {
            ...
            const r = await mediaFetch(`https://generativelanguage.googleapis.com/v1beta/models/${row.model_id}:generateContent?key=${encodeURIComponent(key ?? '')}`, 'google', {
                method: 'POST',
            ...
            if (!b64)
                throw new MediaError('gemini returned no audio', 502);
...
function resolveMediaChain(model, modality) {
...
function logMedia(row, status, latencyMs, error) {
    try {
        getDb()
...
function chainError(modality, lastError) {
    return new MediaError(`All ${modality} providers failed${lastError ? ` (last: ${lastError.message.slice(0, 160)})` : ' (no usable keys)'}.`, lastError && lastError.status === 429 ? 429 : 502);
}
/** Generate image(s), failing over across providers serving the modality. */
export async function runImageGeneration(model, params) {
    ...
    for (const row of chain) {
        ...
        if (!keyless && !key)
            continue; // no usable key for this provider — try the next
...
export async function runSpeech(model, params) {
