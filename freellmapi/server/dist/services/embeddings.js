...
function getPlatformKey(platform) {
    const row = getDb().prepare("SELECT encrypted_key, iv, auth_tag FROM api_keys WHERE platform = ? AND enabled = 1 AND status IN ('healthy', 'unknown') ORDER BY id LIMIT 1").get(platform);
    if (!row)
        return null;
    try {
...
async function openAiStyleEmbed(url, key, modelId, inputs, extra = {}) {
...
async function callProvider(row, key, inputs) {
    switch (row.platform) {
        case 'google':
...
function logEmbeddingRequest(row, status, inputTokens, latencyMs, error) {
    try {
        getDb().prepare(`
...
export async function runEmbeddings(model, inputs) {
