...
export const CLAUDE_FAMILIES = ['default', 'opus', 'sonnet', 'haiku'];
const DEFAULT_MAP = { default: 'auto', opus: 'auto', sonnet: 'auto', haiku: 'auto' };
...
export function getClaudeModelMap() {
...
export function setClaudeModelMap(input) {
...
export function classifyClaudeFamily(model) {
    const m = (model ?? '').trim().toLowerCase();
    if (!m || m === 'auto' || m === 'default' || m === 'freellmapi-auto')
        return 'default';
    if (m.includes('opus'))
        return 'opus';
    ...
    if (m.startsWith('claude') || m === 'opusplan' || m === 'opusplan-4')
        return 'default';
    return null;
}
...
export function resolveAnthropicModel(model) {
    const db = getDb();
    const lookupEnabled = (modelId) => {
        const row = db.prepare('SELECT id FROM models WHERE model_id = ? AND enabled = 1').get(modelId);
        return row?.id;
    };
...
//# sourceMappingURL=anthropic-map.js.map