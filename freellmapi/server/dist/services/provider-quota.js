...
export function runWithQuotaObservationContext(context, fn) {
    return contextStore.run(context, fn);
}
export function getQuotaObservationContext() {
...
function parseResetAtFromHeader(raw, now = Date.now()) {
...
function pickBetterSource(existing, next) {
    if (!existing)
        return next;
    return SOURCE_PRIORITY[next] >= SOURCE_PRIORITY[existing] ? next : existing;
}
function inferPoolForPlatform(platform, modelId) {
...
function isSharedPool(platform) {
...
function maybeAddObservation(observations, base, metric, limitRaw, remainingRaw, resetRaw, strategy) {
...
export function inferQuotaPoolKey(platform, modelId) {
    return inferPoolForPlatform(platform, modelId);
}
export function parseQuotaObservationsFromResponse(response, opts = {}) {
...
export function recordQuotaObservation(input) {
    const context = getQuotaObservationContext();
    ...
    SELECT confidence, notes, source
     ...
     WHERE platform = ?
       ...
       AND quota_pool_key = ?
       AND metric = ?
  `).get(platform, keyId, quotaPoolKey, metric);
    ...
    `).run(platform, keyId, quotaPoolKey, metric, limitValue, remainingValue, resetAt, resetStrategy, source, nextConfidence, nextNotes, nowSql, updatedAt);
        db.prepare(`
    ...
    `).run(id, platform, keyId, providerAccountId, modelId, quotaPoolKey, metric, statusCode, limitValue, remainingValue, resetAt, retryAfterMs, resetStrategy, source, confidence, notes, rawJson, endpoint, nowSql, nowSql);
    })();
    return {
        id,
        platform,
...
export function recordQuotaObservationsFromResponse(response, opts = {}) {
    return parseQuotaObservationsFromResponse(response, opts)
        .map(recordQuotaObservation)
        .filter((row) => row !== null);
}
export function getQuotaStateForKeys() {
    let db;
    try {
        db = getDb();
    }
