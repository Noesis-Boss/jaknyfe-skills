...
function getActiveChain(db) {
    const activeProfileSetting = db.prepare("SELECT value FROM settings WHERE key = 'active_profile_id'").get();
 ...
 * pre-checks.
 */
function selectKeyForModel(entry, estimatedTokens, skipKeys) {
    const db = getDb();
    ...
    for (let attempt = 0; attempt < keys.length; attempt++) {
        ...
        if (entry.platform === 'custom' && entry.key_id != null && key.id !== entry.key_id)
            continue;
...
export function getOrderedFusionChain() {
    ...
    const usableKeys = db.prepare("SELECT id, platform FROM api_keys WHERE enabled = 1 AND status IN ('healthy', 'unknown')").all();
    ...
    for (const k of usableKeys) {
        ...
        if (arr)
            ...
            keysByPlatform.set(k.platform, [k.id]);
    }
    const servable = chain.filter(e => {
        ...
        return keyIds.some(kid => (e.key_id == null || kid === e.key_id) &&
...
export function routeRequest(estimatedTokens = 1000, skipKeys, preferredModelDbId, requireVision = false, requireTools = false, skipModels, prefetchedChain) {
    const db = getDb();
    const strategy = getRoutingStrategy();
    if (strategy !== 'priority')
        refreshStatsCache(db);
