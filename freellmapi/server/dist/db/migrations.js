...
function ensureRequestRequestedModelColumn(db) {
...
function ensureApiKeysBaseUrlColumn(db) {
...
function ensureModelsKeyIdColumn(db) {
...
function migrateModelsV16Vision(db) {
    const columns = db.prepare('PRAGMA table_info(models)').all();
...
function migrateModelsV17IntelligenceTiers(db) {
    const apply = db.transaction(() => {
...
function migrateModelsV18OpenCodeZen(db) {
    const insert = db.prepare(`
    INSERT OR IGNORE INTO models (platform, model_id, display_name, intelligence_rank, speed_rank, size_label, rpm_limit, rpd_limit, tpm_limit, tpd_limit, monthly_token_budget, context_window)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
 ...
 */
function migrateModelsV19Gemma4(db) {
    const insert = db.prepare(`
    INSERT OR IGNORE INTO models (platform, model_id, display_name, intelligence_rank, speed_rank, size_label, rpm_limit, rpd_limit, tpm_limit, tpd_limit, monthly_token_budget, context_window)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
 ...
 */
function migrateModelsV20KiloFree(db) {
    const insert = db.prepare(`
    INSERT OR IGNORE INTO models (platform, model_id, display_name, intelligence_rank, speed_rank, size_label, rpm_limit, rpd_limit, tpm_limit, tpd_limit, monthly_token_budget, context_window)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
 ...
 */
function migrateModelsV21PruneDead(db) {
    const dead = [
        ['llm7', 'gpt-oss-20b'],
    ...
    const apply = db.transaction(() => {
        ...
        for (const [platform, modelId] of dead) {
            const row = getId.get(platform, modelId);
            if (!row)
                continue;
...
// upgrades alike.
function migrateModelsV22Tools(db) {
    const columns = db.prepare('PRAGMA table_info(models)').all();
 ...
 */
function migrateModelsV23FreeTierAudit(db) {
    const apply = db.transaction(() => {
 ...
 */
function migrateModelsV24ZenRefresh(db) {
    const apply = db.transaction(() => {
        const insert = db.prepare(`
      INSERT OR IGNORE INTO models (platform, model_id, display_name, intelligence_rank, speed_rank, size_label, rpm_limit, rpd_limit, tpm_limit, tpd_limit, monthly_token_budget, context_window, enabled, supports_vision, supports_tools)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
 ...
 * disables.
 */
function migrateModelsV25ZenDeadPromos(db) {
    const disable = db.prepare(`UPDATE models SET enabled = 0 WHERE platform = ? AND model_id = ?`);
    const disables = [
...
function migrateEmbeddingsV1(db) {
    db.exec(`
  ...
  `);
    ...
    const apply = db.transaction(() => { for (const r of rows)
        seed.run(...r); });
    apply();
...
function migrateQuirksV1(db) {
  ...
  `);
    ...
    const addTarget = db.prepare('INSERT INTO quirk_targets (quirk_id, platform, model_glob) VALUES (?, ?, ?)');
    const apply = db.transaction(() => {
        for (const s of seeds) {
            upsertQuirk.run({ slug: s.slug, title: s.title, body: s.body, severity: s.severity, now });
            const { id } = getId.get(s.slug);
            ...
            for (const t of s.targets)
                addTarget.run(id, t.platform ?? null, t.modelGlob ?? null);
        }
    });
    apply();
}
 ...
 */
function migrateProfilesInit(db) {
  ...
  `).run();
    ...
    const hasDefault = db.prepare("SELECT COUNT(*) as cnt FROM profiles WHERE type = 'default'").get();
    if (hasDefault.cnt === 0) {
        const minOrder = db.prepare('SELECT COALESCE(MIN(sort_order), 0) AS mn FROM profiles').get().mn;
        const targetOrder = Math.min(-1, minOrder - 1);
        const result = db.prepare("INSERT INTO profiles (name, emoji, color, type, sort_order) VALUES ('Default', '⚙️', '#6366f1', 'default', ?)").run(targetOrder);
