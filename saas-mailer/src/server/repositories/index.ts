...
export function repositories(context: RepositoryContext) {
  ...
  return {
    ...
    campaigns: {
      ...
      insert: (name: string, campaignType = "sequence") => scoped(db => one(db, "INSERT INTO campaigns (id, organization_id, name, campaign_type) VALUES ($1,$2,$3,$4) RETURNING *", [randomUUID(), organizationId, name, campaignType])),
      ...
      firstStep: (campaignId: string) => scoped(db => one<{ subject: string; body: string }>(db, "SELECT subject, body FROM campaign_steps WHERE campaign_id = $1 AND organization_id = $2 ORDER BY step_order LIMIT 1", [campaignId, organizationId])),
      enrolledContacts: (campaignId: string) => scoped(db => rows<{ contact_id: string }>(db, "SELECT contact_id FROM campaign_contacts WHERE campaign_id = $1 AND organization_id = $2", [campaignId, organizationId])),
      schedule: (id: string, scheduledAt: string) => scoped(db => one(db, "UPDATE campaigns SET scheduled_at = $1 WHERE organization_id = $2 AND id = $3 RETURNING *", [scheduledAt, organizationId, id])),
    },
    messages: {
      ...
      insertQueued: (input: { campaignId: string; contactId: string; sendingAccountId: string; idempotencyKey: string; subject: string; body: string; nextAttemptAt: string }) => scoped(db => one(db, "INSERT INTO messages (id, organization_id, campaign_id, contact_id, sending_account_id, status, idempotency_key, subject, body, next_attempt_at) VALUES ($1,$2,$3,$4,$5,'queued',$6,$7,$8,$9) ON CONFLICT (idempotency_key) DO NOTHING RETURNING *", [randomUUID(), organizationId, input.campaignId, input.contactId, input.sendingAccountId, input.idempotencyKey, input.subject, input.body, input.nextAttemptAt])),
