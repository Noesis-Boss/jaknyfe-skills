...
import { connectSendingAccount } from "../sending/service";
...
export function createProviderCallbackRoutes(database: Database, config: AppConfig): Hono {
  const routes = new Hono();
  routes.get("/api/oauth/:provider/start", c => {
    const provider = c.req.param("provider");
    if (!isProvider(provider) || !config.oauth.callbackOrigin) return c.json({ error: "OAuth provider is not configured" }, 400);
    try {
      const tenant = requireTenant(database, c.req.raw);
  ...
  routes.get("/api/oauth/:provider/callback", async c => {
    ...
    try {
      ...
      connectSendingAccount(database, tenant.organizationId, { provider, email, credentials: tokens });
