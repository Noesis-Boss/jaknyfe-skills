...
export function createCampaignRoutes(database: Database | PostgresDatabase): Hono {
  ...
  routes.get("/api/campaigns", async c => { try { const tenant = isPostgres(database) ? await requireTenantPostgres(database, c.req.raw) : requireTenant(database, c.req.raw); return c.json(isPostgres(database) ? { campaigns: await listCampaignsPostgres(database, tenant.organizationId) } : { campaigns: listCampaigns(database, tenant.organizationId) }); } catch { return c.json({ error: "Unable to list campaigns" }, 400); } });
