...
import { loadQueueEligibility } from "../src/server/campaigns/eligibility";
...
test("hard bounce records bounce event and suppresses future sends", async () => { registerSendingAdapter("bounce", { async send() { throw Object.assign(new Error("550 5.1.1 user unknown"), { code: "permanent_failure" }); } }); const { db, campaign } = fixture(); execute(db, "UPDATE sending_accounts SET provider = 'bounce' WHERE id = 'a1'"); const result = await processQueuedSend(db, job(campaign.id)); expect(result.status).toBe("permanent_failure"); expect(db.query("SELECT type FROM events WHERE type = 'bounce'").get()).toEqual({ type: "bounce" }); expect(loadQueueEligibility(db, "org-a", campaign.id, "c1")).toEqual({ eligible: false, reason: "bounced" }); });
