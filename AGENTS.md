...
- 2026-09-06: Key-leak incident closed out. Root leak: `freellmapi/.env` + `freellmapi/server/.env` (Anthropic sk-ant, OpenAI sk-proj, OpenRouter sk-or-v1, GCP, GitHub fine-grained PAT) committed to jaknyfe-skills history and pushed to GitHub; second copy in `recovered-2026-08-27/.../bound-by-ash-technical-debt/local-live/.env`. Provider keys now disabled by user. History: leaked files purged from pushed refs where feasible; remaining master-lineage findings are documented in `.gitleaksignore` with "rotation pending" notes — dead keys remain in remote history (hygiene item, not urgent). Local stash (substrate-bot, 140 secrets) extracted to `Archive/stash-recovery-2026-09-06/` and cleared — pushes now work directly from /home/workspace. Pre-push gitleaks hook + Zo rule enforce no-key pushes. gitleaks v8.28.0 installed at /usr/local/bin.
